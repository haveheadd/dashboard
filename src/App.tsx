  ...
  return <div className="app">
    ...
    <main>{!sidebar && <button className="open-side" onClick={() => setSidebar(true)}>B</button>}
      {page === 'team' ? <Team people={people} setPeople={setPeople} tasks={tasks} notify={notify} /> : <>
        ...
        <section className="import-panel" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); const file = event.dataTransfer.files[0]; if (file) void importProject(file); }}>
          <div className="import-panel-icon"><Upload /></div>
          <div><b>Импортировать свой проект</b><p>Перетащите сюда Excel, CSV или JSON — Гант построится автоматически</p></div>
          <label className="import-panel-button">Выбрать файл<input type="file" accept=".xlsx,.xls,.csv,.json" onChange={event => { const file = event.target.files?.[0]; if (file) void importProject(file); event.target.value = ''; }} /></label>
        </section>
